...
public:
    ...
    void addNode(int node1, int node2) {
        ...
        if (temp->next != nullptr && (temp->next->val == node2) || temp->val == node2) {
        ...
        temp = graph[node2];
        while (temp->next != nullptr && temp->next->val < node1) {
            temp = temp->next;
        }
        node = temp->next;
        temp->next = new GraphNode(node1);
        temp->next->next = node;
    }
...
int main()
{
    std::cout << "------------ Adjacency List for Undirected Graph --------------" << endl << endl;
